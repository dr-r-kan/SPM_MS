...
%RUN_PART1_SIMULATED_CLUSTER Cluster entry point for the extensive simulation study.
    ...
    success_marker = fullfile(output_dir, 'PART1_CLUSTER_RUN_PASSED.txt');
    failure_marker = fullfile(output_dir, 'PART1_CLUSTER_RUN_FAILED.txt');
    ...
    diary_file = fullfile(log_dir, ['part1_matlab_' datestr(now, 'yyyymmdd_HHMMSS') '.log']);
    diary(diary_file);
    ...
    try
        ...
        assert(isfile(results_csv), 'Part 1 run did not produce %s', results_csv);
        ...
        if fid >= 0
            fprintf(fid, 'Part 1 cluster run passed at %s\n', datestr(now, 'yyyy-mm-dd HH:MM:SS'));
        ...
        if fid >= 0
            fprintf(fid, 'Part 1 cluster run failed at %s\n', datestr(now, 'yyyy-mm-dd HH:MM:SS'));
            fprintf(fid, 'Message: %s\n', ME.message);
            fprintf(fid, 'Identifier: %s\n', ME.identifier);
            fprintf(fid, 'MATLAB diary: %s\n', diary_file);
            fclose(fid);
        end
        fprintf(2, 'Part 1 cluster run failed. Wrote failure marker: %s\n', failure_marker);
        rethrow(ME);
    end
end
